...
# Import des donnees Marketing.csv
marketing_data = pd.read_csv("../Marketing.csv", encoding='latin-1')

# Preparation des donnees pour la prediction
###########################################################################################
# 1 pour Homme et 0 pour Femme
marketing_data.loc[(marketing_data.sexe == 'M'),'sexe'] = 1
marketing_data.loc[(marketing_data.sexe == 'F'),'sexe'] = 0

# 1 pour En Couple et 0 pour Celibataire
marketing_data.loc[(marketing_data.situationFamiliale == 'En Couple'),'situationFamiliale'] = 1
marketing_data.loc[(marketing_data.situationFamiliale == 'Célibataire'),'situationFamiliale'] = 0

# 1 pour true et 0 pour false
marketing_data.loc[(marketing_data['2eme voiture'] == True),'2eme voiture'] = 1
marketing_data.loc[(marketing_data['2eme voiture'] == False),'2eme voiture'] = 0
###########################################################################################

# Application de la prediction des donnees
X_test = marketing_data.values
predictions = clf.predict(X_test)

for i in range(len(marketing_data)):
    print(str(marketing_data.iloc[i]) + " - Prediction : " + str(predictions[i]) + "\n")